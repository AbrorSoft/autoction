...
type RestOf<T extends IProduct | NewProduct> = Omit<T, 'producedYear' | 'auctionDate'> & {
...
export class ProductService {
  ...
  protected convertDateFromClient<T extends IProduct | NewProduct | PartialUpdateProduct>(product: T): RestOf<T> {
    return {
      ...product,
      producedYear: product.producedYear?.toJSON() ?? null,
      auctionDate: product.auctionDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restProduct: RestProduct): IProduct {
    return {
      ...restProduct,
      producedYear: restProduct.producedYear ? dayjs(restProduct.producedYear) : undefined,
      auctionDate: restProduct.auctionDate ? dayjs(restProduct.auctionDate) : undefined,
    };
  }
